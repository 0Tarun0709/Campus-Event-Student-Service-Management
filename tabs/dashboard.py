...
def dashboard():
    ...
    with col3:
        st.metric("Active Service Requests", len(st.session_state.system.service_requests))
    ...
    if event_data:
        ...
        fig = px.bar(df, x='Event', y=['Confirmed', 'Waitlisted', 'Available'], 
                     title='Event Registration Status', barmode='stack')
        st.plotly_chart(fig)