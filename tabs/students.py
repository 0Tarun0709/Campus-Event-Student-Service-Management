...
def manage_students():
    ...
    if st.session_state.system.students:
        ...
        for student in st.session_state.system.students.values():
            registrations = len([r for r in student.registrations if r.status == RegistrationStatus.CONFIRMED])
            waitlisted = len([r for r in student.registrations if r.status == RegistrationStatus.WAITLISTED])
            student_data.append({
