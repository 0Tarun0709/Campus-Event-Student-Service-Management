    ...
    with st.expander("Raise New Service Request"):
        ...
        with col1:
            ...
            student_id = st.selectbox(
                "Student ID", options=list(st.session_state.system.students.keys())
            )
        with col2:
            category = st.selectbox(
                "Category",
                ["Hostel Maintenance", "Library Access", "Counseling", "Other"],
            )

        if st.button("Submit Request"):
            if all([request_id, student_id, category]):
                request = st.session_state.system.raise_service_request(
                    request_id, student_id, category
                )
                if request:
            ...
            with st.container():
                ...
                with col3:
                    ...
                    new_status = st.selectbox(
                        ...
                        index=[status.value for status in RequestStatus].index(
                            current_status
                        ),
                    )
                    if new_status != current_status:
                        st.session_state.system.update_service_request_status(
                            request.request_id, RequestStatus(new_status)
                        )
                with col4:
